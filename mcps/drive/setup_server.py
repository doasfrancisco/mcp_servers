"""Web-based setup for Google Drive MCP Server.

Starts a local HTTP server that guides users through adding Drive accounts
...
DRIVE_DIR = Path(__file__).parent
CREDENTIALS_PATH = DRIVE_DIR / "credentials" / "credentials.json"
ACCOUNTS_PATH = DRIVE_DIR / "accounts.json"
SCOPES = ["https://www.googleapis.com/auth/drive"]

# ── State ──────────────────────────────────────────────────────────────
_port: int = 0
_accounts: list[dict] = []
_pending_creds: dict = {}  # email → creds waiting for alias
...
def _main_page() -> bytes:
    ...
    return _page("Drive MCP Setup", f"""
        <h1>Google Drive MCP Setup</h1>
        <p class="sub">Add your Google Drive accounts to get started.</p>
        {accounts_section}
        <div class="actions">
            <a href="/add" class="btn btn-primary">Add Google account</a>
            {finish_btn}
        </div>
    """)
...
def _done_page(default_alias: str) -> bytes:
    drive_dir = str(DRIVE_DIR).replace("\\", "/")
    claude_code_cmd = f'claude mcp add -s user drive -- uv run --directory "{drive_dir}" fastmcp run server.py'
    desktop_config = json.dumps({
        "mcpServers": {
            "drive": {
                "command": "uv",
                "args": ["run", "--directory", drive_dir, "fastmcp", "run", "server.py"],
            }
        }
    }, indent=2)
    ...
    def do_GET(self):
        ...
        if path == "/":
            ...
            try:
                ...
                # Drive API: get user email via about().get()
            ...
            if email and alias and email in _pending_creds:
                ...
                token_path = DRIVE_DIR / "credentials" / f"token_{alias}.json"
...
if __name__ == "__main__":
    if not needs_setup():
        print("Drive MCP is already configured.")
        config = json.loads(ACCOUNTS_PATH.read_text())
